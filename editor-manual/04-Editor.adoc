...
The _Status_ editor page is where the user manages the message status.

At the left side of the page, the user may preview the message details in all supported
model languages.

At the right side, there is a status panel with the current status of the message, and buttons
for manually changing the message status.
...
When a verified draft is published by clicking the "Publish Message" button, and if the message contain
